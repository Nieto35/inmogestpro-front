...
// Tipos de interacción — diferenciados solo por ícono, color de marca
// El manual define 3 colores: navy, gold, marfil. Sin colores decorativos extra.
const TYPES = {
  llamada:  { label:'Llamada',  Icon:Phone,         color:'#0D1B3E' }, // navy
  whatsapp: { label:'WhatsApp', Icon:MessageSquare, color:'#0D1B3E' }, // navy
  email:    { label:'Email',    Icon:Mail,          color:'#C8A84B' }, // gold — acción/distinción
    ...
    <div>
      ...
      <div className="space-y-1.5 max-h-40 overflow-y-auto">
        <button
          ...
          style={{
            background: !selected ? 'rgba(13,27,62,0.07)' : 'var(--color-bg-secondary)',
        ...
        {contracts.map(c => {
          ...
          const isMora     = c.status === 'en_mora' || (parseInt(c.overdue_count||0) > 0);
          // Colores semánticos de estado del contrato — se mantienen
          const color = c.status === 'cancelado' ? 'var(--color-text-muted)'
                      : isMora   ? 'var(--color-danger)'
                      : c.status === 'escriturado' ? 'var(--color-navy)'
                      : 'var(--color-success)';
          ...
          return (
            <button key={c.id}
              ...
              className="w-full text-left px-3 py-2 rounded text-xs transition-all"
              style={{
                background: isSelected ? `${color}12` : 'var(--color-bg-secondary)',
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-mono font-semibold"
                    style={{ color: isSelected ? color : 'var(--color-gold)' }}>
                    {c.contract_number}
                  </span>
      ...
      {selected && (
        <p className="text-xs mt-1.5" style={{ color:'var(--color-text-muted)' }}>
          ✓ Contrato seleccionado: <span style={{ color:'var(--color-gold)' }}>{selected.contract_number}</span>
        </p>
      )}
    </div>
  );
...
const NewInteractionModal = ({ onClose, onSaved, preselectedClient = null, preselectedContract = null, tenant }) => {
  ...
  const selectClient = (c) => { setClient(c); setSearch(c.full_name); setShowDrop(false); };
  const clearClient  = ()  => { setClient(null); setSearch(''); };

  const handleSubmit = async () => {
    if (!client) return toast.error('Debe seleccionar un cliente');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center"
      ...
      <div className="w-full rounded-xl shadow-2xl"
        ...
        {/* Header — navy + línea dorada */}
        <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
          style={{ background:'var(--color-navy)', borderBottom:'3px solid var(--color-gold)' }}>
          <h2 className="font-bold text-base" style={{ color:'#F5F3EE', fontFamily:'var(--font-display)' }}>
        ...
        <div className="px-5 py-4 space-y-5 overflow-y-auto flex-1">
          ...
          <div>
            <label className="block text-sm font-semibold mb-2"
              style={{ color:'var(--color-text-primary)' }}>
              1. Cliente <span style={{ color:'var(--color-danger)' }}>*</span>
            </label>
            {!client ? (
              <div className="relative">
                ...
                {showDrop && results.length > 0 && (
                  <div className="absolute z-30 w-full mt-1 rounded overflow-hidden shadow-2xl"
              ...
              /* Cliente seleccionado — navy en lugar de verde */
              <div className="flex items-center justify-between px-4 py-3 rounded"
                style={{ background:'rgba(13,27,62,0.05)', border:'1px solid var(--color-border)', borderLeft:'3px solid var(--color-gold)' }}>
                <div className="flex items-center gap-2">
                ...
                <button onClick={clearClient} style={{ color:'var(--color-danger)' }} className="p-1 rounded">
                  <X size={14}/>
                </button>
              </div>
            )}
          ...
          {/* 2. Contrato */}
          {client && (
            <ContractSelector clientId={client.id} selected={contract} onSelect={setContract}/>
          )}
          ...
          <div>
            ...
            <div className="grid grid-cols-5 gap-2">
              {Object.entries(TYPES).map(([k, v]) => {
                ...
                return (
                  <button key={k} onClick={() => set('interaction_type', k)}
                    className="flex flex-col items-center gap-1.5 py-2.5 rounded text-xs font-medium transition-all"
                    style={{
                      background: sel ? `${v.color}20` : 'var(--color-bg-secondary)',
                      border:     `1.5px solid ${sel ? v.color : 'var(--color-border)'}`,
                      color:      sel ? v.color : 'var(--color-text-secondary)',
                    }}>
                    <v.Icon size={16}/>
                    {v.label}
                  </button>
          ...
          <div>
            <label className="block text-sm font-semibold mb-2"
              style={{ color:'var(--color-text-primary)' }}>
              4. Resumen <span style={{ color:'var(--color-danger)' }}>*</span>
            </label>
            <textarea
              value={form.summary}
            ...
            <p className="text-xs mt-1"
              style={{ color: form.summary.length >= 5 ? 'var(--color-navy)' : 'var(--color-text-muted)' }}>
              {form.summary.length} caracteres {form.summary.length >= 5 ? '✓' : '(mínimo 5)'}
            </p>
          </div>

          {/* 5. Resultado y próximo contacto */}
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-semibold mb-2"
                style={{ color:'var(--color-text-primary)' }}>5. Resultado</label>
              <select value={form.outcome} onChange={e => set('outcome', e.target.value)} className="input text-sm">
            ...
            <div>
              <label className="block text-sm font-semibold mb-2"
                style={{ color:'var(--color-text-primary)' }}>Próximo contacto</label>
              <input type="date" value={form.next_contact}
                onChange={e => set('next_contact', e.target.value)} className="input text-sm"/>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 pb-4">
          ...
          <label
            className="flex items-center gap-2 px-4 py-3 rounded cursor-pointer transition-all w-full"
            style={{
            ...
            <div className="mt-2 space-y-1.5">
              {attachedFiles.map((file, idx) => {
                ...
                const icon = ['jpg','jpeg','png','webp'].includes(ext) ? '🖼️'
                           ...
                           : ext === 'mp4' ? '🎬' : '📎';
                return (
                  <div key={idx} className="flex items-center justify-between px-3 py-2 rounded text-xs"
                    ...
                    <button
                      ...
                      className="ml-2" style={{ color:'var(--color-danger)' }}>
                      <X size={12}/>
                    </button>
                  </div>
                );
            ...
            <div key={day}>
              ...
              <div className="flex items-center gap-3 mb-3">
                ...
                <span className="text-xs font-semibold uppercase tracking-wider px-3 py-1 rounded-full"
                  style={{ color:'var(--color-text-muted)', background:'var(--color-bg-secondary)', border:'1px solid var(--color-border)' }}>
                  {day}
                </span>
                <div className="h-px flex-1" style={{ background:'var(--color-border)' }}/>
              </div>
                  ...
                  return (
                    <div key={i.id} className="card hover:shadow-md transition-all">
                      <div className="flex items-start gap-3">
                        ...
                        {/* Ícono tipo — fondo con opacidad suficiente para fondo claro */}
                          ...
                          {i.documents && i.documents.length > 0 && (
                            <div className="flex flex-wrap gap-2 mt-2">
                              {i.documents.map((doc, di) => (
                                <div key={di} className="flex items-center gap-1.5 px-2 py-1 rounded text-xs"
                                  style={{ background:'rgba(13,27,62,0.05)', border:'1px solid var(--color-border)' }}>
                            ...
                            <label className="inline-flex items-center gap-1.5 mt-2 px-2 py-1 rounded text-xs cursor-pointer transition-colors hover:opacity-80"
                              ...
                              <input type="file" className="hidden"
                                ...
                                onChange={async e => {
                                  ...
                                  try {
                                    const token = localStorage.getItem('inmogest_token');
                                    const apiBaseLocal = () => `${API_URL}/api/v1/${tenant}`;
                                    const res = await fetch(`${apiBaseLocal()}/interactions/${i.id}/upload`, {
                                      method:'POST', headers:{ Authorization:`Bearer ${token}` }, body:fd,
                                    });
                                    const json = await res.json();
                                    if (json.success) { toast.success('Evidencia subida correctamente'); refetch(); }
