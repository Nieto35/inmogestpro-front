...
import { ArrowLeft, Plus, Trash2, Home, AlertTriangle } from 'lucide-react';
...
const PropertyBulkPage = () => {
  ...
  const handleSave = async () => {
    ...
    if (overLimit) return toast.error(`Excedes el límite: solo quedan ${available} cupos en este proyecto`);
    ...
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm" style={{ color:'var(--color-text-muted)' }}>
            Crea múltiples inmuebles similares en un solo paso
          </p>
        </div>
      ...
      <div className="card">
        ...
        <select value={projectId} onChange={e => setProjectId(e.target.value)}
          className="input text-sm" style={{ maxWidth:'400px' }}>
          ...
          {projects.map(p => {
            ...
            return (
              <option key={p.id} value={p.id} disabled={full}>
                {p.name} ({p.code})
                {avail !== null
                  ? full ? ' — COMPLETO' : ` — ${avail} cupo${avail !== 1 ? 's' : ''} disponible${avail !== 1 ? 's' : ''}`
                  : ''}
              </option>
            );
          })}
        ...
        {selectedProject && maxUnits > 0 && (
          <div className={`mt-3 px-4 py-3 rounded-xl text-sm flex items-center gap-2`}
            ...
            {overLimit
              ? <><AlertTriangle size={15}/> Excedes el límite: quieren crear {totalUnits} pero solo hay {available} cupos disponibles en este proyecto.</>
              : available === 0
              ? <>✅ Proyecto completo — todas las {maxUnits} unidades están creadas.</>
              : <>📦 Puedes crear hasta <strong>{available}</strong> inmueble{available !== 1 ? 's' : ''} más en este proyecto (tope: {maxUnits} total, actuales: {currentProp}).</>}
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs" style={{ color:'var(--color-text-muted)' }}>
              Total a crear: <strong style={{ color: overLimit ? '#ef4444' : 'var(--color-text-primary)' }}>{totalUnits}</strong>
            </span>
      ...
      <div className="flex justify-end gap-3 pb-8">
        ...
        <button onClick={handleSave}
          disabled={saving || !projectId || overLimit || totalUnits === 0}
          className="btn btn-primary">
          <Home size={15}/>
